      ...
      <div class="flex flex-col md:flex-row justify-between items-start gap-8 mb-16">
        <div class="flex items-start gap-6">
          ...
          <div>
            ...
            <p
              :if={@app.description}
              class="text-base-content/50 mt-4 text-sm font-medium leading-relaxed italic border-l-2 border-base-200 pl-4 max-w-2xl"
            >
              {@app.description}
            </p>
          </div>
        ...
        <div class="flex items-center gap-2 self-center md:self-auto">
          <%= if @can_edit do %>
            <.link
              navigate={~p"/workspaces/#{@current_workspace.id}/apps/#{@app.id}/edit"}
              class="btn btn-primary btn-sm rounded-lg px-6 font-black uppercase text-[10px] tracking-widest shadow-lg shadow-primary/20"
            >
              <.icon name="hero-pencil" class="w-3.5 h-3.5 mr-2" /> Update Spec
            </.link>
            <button
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <%= for feature <- @app.features do %>
                <div
                  phx-click={
                    JS.navigate(
                      ~p"/workspaces/#{@current_workspace.id}/apps/#{@app.id}/features/#{feature.id}/tasks"
                    )
                  }
                ...
                >
                  <div class="flex items-center gap-4 mb-4 relative z-10">
                    ...
                    <div class="flex-1 min-w-0">
                      <h4 class="font-black text-base-content group-hover:text-primary transition-colors truncate tracking-tight">
                        {feature.title}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center justify-between pt-4 border-t border-base-50 mt-auto z-10 relative">
                    <span class="text-[9px] font-black uppercase text-base-content/20 tracking-widest italic group-hover:text-primary transition-colors">
                      Project Scope
                    </span>
        ...
        <aside class="space-y-8">
          <div class="bg-base-50/50 border border-base-200 rounded-xl p-8 space-y-8">
            <div class="space-y-6">
              <div>
                ...
                <div class="flex items-center gap-3 px-4 py-3 bg-base-100 rounded-lg border border-base-200 shadow-sm font-medium">
                  <div class="w-8 h-8 rounded-lg bg-primary/10 text-primary flex items-center justify-center text-[11px] font-black uppercase">
                    {if @app.last_updated_by,
                      do: String.at(@app.last_updated_by.email, 0) |> String.upcase(),
                      else: "U"}
                  </div>
                  <div class="flex flex-col min-w-0">
                    <span class="text-xs font-black text-base-content truncate">
                      {if @app.last_updated_by, do: @app.last_updated_by.email, else: "Unknown"}
                    </span>
                    <span class="text-[9px] font-bold text-base-content/30 uppercase tracking-widest">
                      Editor
                    </span>
                  </div>
                </div>
              ...
              <div class="bg-base-100 rounded-lg p-6 border border-base-200 shadow-sm space-y-4">
                <h4 class="text-[10px] font-black uppercase tracking-widest text-base-content/20">
                  Spec Info
                </h4>
                <div class="space-y-1">
                  <div class="flex items-center justify-between p-2 rounded bg-base-50 border border-base-100 text-[9px] font-bold">
                    ...
                    <span class="text-base-content/70">
                      {Calendar.strftime(@app.inserted_at, "%b %Y")}
                    </span>
                  </div>
                  <div class="flex items-center justify-between p-2 rounded bg-base-50 border border-base-100 text-[9px] font-bold">
                    <span class="text-base-content/40 uppercase">Updated</span>
                    <span class="text-base-content/70">
                      {Calendar.strftime(@app.updated_at, "%b %d")}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div class="px-8 flex items-center justify-between">
            <span class="text-[9px] font-black text-base-content/20 uppercase tracking-widest">
              Members
            </span>
            <div class="flex -space-x-1.5">
              <%= for member <- @workspace_members do %>
                <div
                  class="w-6 h-6 rounded bg-primary ring-2 ring-base-100 flex items-center justify-center text-[8px] font-black text-white uppercase shadow-sm"
              ...
              <% end %>
              <div
                :if={length(@workspace_members) > 5}
                class="w-6 h-6 rounded bg-base-200 ring-2 ring-base-100 flex items-center justify-center text-[8px] font-black text-base-content/40 shadow-sm"
              >
    ...
    if is_nil(current_workspace) do
      ...
      case Planner.get_app(user, id, current_workspace.id) do
        ...
        app ->
          ...
          workspace_members = Workspaces.list_workspace_members(current_workspace.id)
