defmodule AppPlannerWeb.TaskLive.Form do
  ...
  def mount(params, _session, socket) do
    ...
    assignees = Enum.map(workspace_members, fn u -> {u.email, u.id} end)
      ...
      <div class="mb-10 flex items-center justify-between">
        <div>
           <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-base-content/30 mb-2">
              <.link navigate={~p"/workspaces/#{@workspace_id}/apps/#{@app_id}/features/#{@feature.id}/tasks"} class="hover:text-primary transition-colors">Board</.link>
              <span>/</span>
        ...
        <div class="flex items-center gap-2">
           <.link navigate={~p"/workspaces/#{@workspace_id}/apps/#{@app_id}/features/#{@feature.id}/tasks"} class="btn btn-ghost btn-sm rounded-lg border border-base-200 uppercase text-[10px] font-black tracking-widest px-6 transition-all">
              Cancel
           </.link>
           <button form="task-form" type="submit" class="btn btn-primary btn-sm rounded-lg px-8 uppercase text-[10px] font-black tracking-widest shadow-lg shadow-primary/20">
              Save
           </button>
        </div>
      </div>
      ...
      >
        <div class="bg-base-50/50 border border-base-200 rounded-xl p-8 space-y-8">
           <div class="form-control">
              <label class="label"><span class="label-text text-[10px] font-black uppercase tracking-widest text-base-content/40">Task Name</span></label>
              <.input field={@form[:title]} type="text" placeholder="Objective name" required class="input input-bordered w-full rounded-lg bg-base-100 font-bold" />
           </div>
           ...
           <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="form-control">
                 ...
                 <.input field={@form[:category]} type="text" placeholder="e.g. Design" class="input input-bordered w-full rounded-lg bg-base-100 font-bold" />
              </div>
              <div class="form-control">
                 ...
                 <.input field={@form[:status]} type="select" options={@statuses} class="select select-bordered w-full rounded-lg bg-base-100 font-bold" />
              </div>

              <div class="form-control">
                 <label class="label"><span class="label-text text-[10px] font-black uppercase tracking-widest text-base-content/40">Assignee</span></label>
                 <.input field={@form[:assignee_id]} type="select" options={@assignees} prompt="Select Person" class="select select-bordered w-full rounded-lg bg-base-100 font-bold" />
              </div>
              <div class="form-control">
                 <label class="label"><span class="label-text text-[10px] font-black uppercase tracking-widest text-base-content/40">Due Date</span></label>
                 <.input field={@form[:due_date]} type="date" class="input input-bordered w-full rounded-lg bg-base-100 font-bold" />
              </div>

              <div class="form-control">
                 <label class="label"><span class="label-text text-[10px] font-black uppercase tracking-widest text-base-content/40">Estimate</span></label>
                 <.input field={@form[:time_estimate]} type="text" placeholder="e.g. 2 days" class="input input-bordered w-full rounded-lg bg-base-100 font-bold" />
              </div>
           </div>
           ...
           <div class="divider opacity-20"></div>
            ...
            <div class="form-control">
               <label class="label"><span class="label-text text-[10px] font-black uppercase tracking-widest text-base-content/40">Task Requirements</span></label>
               <.input field={@form[:description]} type="textarea" rows={10} placeholder="Describe what needs to be done..." class="textarea textarea-bordered w-full rounded-lg bg-base-100 font-medium leading-relaxed" />
            </div>

            <div class="space-y-4">
               <label class="text-[10px] font-black uppercase tracking-widest text-base-content/40 px-1">Visual Marker</label>
               <div class="flex items-center gap-6 p-4 bg-base-100 rounded-lg border border-base-200">
                  ...
                  <div class="flex-1 min-w-0">
                     <input type="text" phx-keyup="search-icons" placeholder="Filter vectors..." class="input input-sm input-bordered w-full rounded-lg mb-3 bg-base-50 font-bold text-xs" />
         ...
         <div class="flex justify-end gap-3 pt-4">
            <.link navigate={~p"/workspaces/#{@workspace_id}/apps/#{@app_id}/features/#{@feature.id}/tasks"} class="btn btn-ghost rounded-lg uppercase text-[10px] font-black tracking-widest border border-base-200 px-8">
               Cancel
            </.link>
            <button type="submit" phx-disable-with="Syncing..." class="btn btn-primary rounded-lg px-12 uppercase text-[10px] font-black tracking-widest shadow-lg shadow-primary/20">
               Save Task
            </button>
         </div>
