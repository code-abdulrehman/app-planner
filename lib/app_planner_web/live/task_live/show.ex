  ...
  def render(assigns) do
    ...
    <div class="max-w-5xl mx-auto py-12 px-6">
      ...
      <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-base-content/30 mb-10 border-b border-base-200 pb-4">
        ...
        <span>/</span>
        <.link navigate={~p"/workspaces/#{@current_workspace.id}/apps/#{@task.feature.app_id}"} class="hover:text-primary transition-colors">Project</.link>
        <span>/</span>
        <.link navigate={~p"/workspaces/#{@current_workspace.id}/apps/#{@task.feature.app_id}/features/#{@task.feature_id}/tasks"} class="hover:text-primary transition-colors">Board</.link>
        <span>/</span>
       ...
       <div class="flex flex-col lg:flex-row justify-between items-start gap-12 mb-16">
         <div class="flex-1 min-w-0">
            <div class="flex items-center gap-5">
               <div :if={@task.icon} class="w-16 h-16 rounded-xl bg-primary/10 text-primary border border-primary/20 flex items-center justify-center shadow-sm group">
                  <.icon name={"hero-#{@task.icon}"} class="w-8 h-8 group-hover:scale-110 transition-transform" />
               </div>
               <div>
            ...
            <div class="flex items-center gap-6 mt-8">
               <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-lg bg-base-200 border border-base-200 flex items-center justify-center text-[10px] font-black text-base-content/40 uppercase" title={if @task.assignee, do: @task.assignee.email, else: "Unassigned"}>
                     {if @task.assignee, do: String.at(@task.assignee.email, 0) |> String.upcase(), else: "?"}
                  </div>
                  <span class="text-[10px] font-black uppercase text-base-content/40 tracking-widest">{if @task.assignee, do: @task.assignee.email |> String.split("@") |> List.first(), else: "Unassigned"}</span>
               </div>
               ...
               <div class="flex items-center gap-2 text-[10px] font-black uppercase text-base-content/30 tracking-widest">
                  ...
                  {if @task.due_date, do: Calendar.strftime(@task.due_date, "%b %d, %Y"), else: "No Deadline"}
               </div>
               <div :if={@task.time_estimate} class="flex items-center gap-2 text-[10px] font-black uppercase text-primary tracking-widest bg-primary/5 px-3 py-1 rounded border border-primary/10">
         ...
         <div class="flex items-center gap-2 shrink-0">
           <button phx-click="delete" data-confirm="Delete this task?" class="btn btn-ghost btn-sm rounded-lg px-3 text-error hover:bg-error/5">
       ...
       <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
         <div class="lg:col-span-3 space-y-12">
            <section :if={@task.description}>
               <div class="flex items-center gap-3 mb-6">
                  ...
                  <h3 class="text-[10px] font-black uppercase tracking-widest text-base-content/40">Task Requirements</h3>
               </div>
           ...
           <section class="space-y-8">
               <div class="flex items-center gap-3">
                  ...
                  <h3 class="text-[10px] font-black uppercase tracking-widest text-base-content/40">Discussion</h3>
               </div>
                     ...
                     <div class="flex gap-4 group">
                        ...
                        <div class="flex-1 bg-base-50/50 border border-base-200 rounded-xl p-6 relative group/msg">
                           <div class="flex items-center justify-between mb-3">
                              <div class="flex items-center gap-3">
                                 <span class="text-[10px] font-black text-base-content/80">{comment.user.email |> String.split("@") |> List.first()}</span>
               ...
               <form phx-submit="add-comment" class="bg-base-100 p-8 rounded-xl border border-base-200 space-y-4">
                  <textarea
                     name="content"
                  ...
                  <div class="flex justify-end">
                     <button type="submit" class="btn btn-primary btn-sm rounded-lg px-8 text-[9px] font-black uppercase tracking-widest shadow-lg shadow-primary/20">
                        Post Comment
                     </button>
                  </div>
               </form>
        ...
        <aside class="space-y-8">
           <div class="bg-base-50/50 border border-base-200 rounded-xl p-6 space-y-8">
              <div>
                <span class="text-[9px] font-black text-base-content/20 uppercase tracking-widest block mb-4">Resource linkage</span>
                <div class="flex flex-col gap-2">
                   <a :if={@task.git_link} href={@task.git_link} target="_blank" class="flex items-center justify-between p-3 bg-white hover:bg-base-200 rounded-lg border border-base-200 group/link transition-all">
                      <div class="flex items-center gap-2">
                         <.icon name="hero-code-bracket" class="w-3.5 h-3.5 text-base-content/30 group-hover/link:text-primary transition-colors" />
                         <span class="text-[9px] font-black uppercase text-base-content/60 tracking-wider">Source</span>
                      </div>
                      <.icon name="hero-arrow-top-right-on-square" class="w-3 h-3 text-base-content/10" />
                   </a>
                   <div :if={!@task.git_link} class="p-3 rounded-lg border border-dashed border-base-200 text-center italic text-[9px] font-bold text-base-content/20">
                      Unlinked
                   </div>
                </div>
              </div>
              ...
              <div class="bg-white rounded-lg p-5 border border-base-200 shadow-sm space-y-4">
                 <h4 class="text-[9px] font-black uppercase tracking-widest text-base-content/20">Metadata</h4>
                 <div class="space-y-2">
                    <div class="flex items-center justify-between p-2 rounded bg-base-50 border border-base-100 text-[9px] font-bold">
                       ...
                       <span class="text-base-content/70">{Calendar.strftime(@task.inserted_at, "%b %y")}</span>
                    </div>
                    <div class="flex items-center justify-between p-2 rounded bg-base-50 border border-base-100 text-[9px] font-bold">
