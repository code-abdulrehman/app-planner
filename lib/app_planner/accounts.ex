defmodule AppPlanner.Accounts do
  @moduledoc """
  ...
  @doc """
  Returns true if the user has role super_admin.
  """
  def super_admin?(%User{role: "super_admin"}), do: true
  def super_admin?(_), do: false

  @doc """
  Returns an `%Ecto.Changeset{}` for updating user profile (full name).
  """
  ...
  @doc """
  Lists all users (for super admin).
  """
  def list_users do
  ...
  See `AppPlanner.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `AppPlanner.Accounts.User.password_changeset/3` for a list of supported options.
